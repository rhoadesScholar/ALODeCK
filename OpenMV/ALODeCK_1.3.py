...
while(sum(variance) > 18 or len(circles) < 1):
    img = sensor.snapshot()
...
while(True):
    ...
    if maxButt*maxHead > 0:
        ...
        buttTheta = math.atan(buttV[1]/buttV[0])
        buttVO = [math.copysign(buttMagO*math.cos(buttTheta), buttV[0]), math.copysign(buttMagO*math.sin(buttTheta), buttV[1])]
        ...
        dot = sum([a*b for a,b in zip(headV, buttV)])
        difTheta = math.acos(dot/(buttMag*headMag))
        magP = headMagO*math.cos(difTheta)
        if magP > 0 and headMagO > buttMagO:
            out = [math.copysign(magP*math.cos(headTheta), headV[0]), math.copysign(magP*math.sin(headTheta), headV[1])]
