...
while(True):
    ...
    while(sum(variance) > 400 or len(maxAmp) < 10):
        ...
        maxButt = 0
        maxHead = 0
        for blob in img.find_blobs(thresholds, pixels_threshold=6, area_threshold=8):
        ...
        if maxButt*maxHead > 0:
            ...
            if headMagO > buttMagO:
                ...
                magP = dot/bodyMag
                maxAmp.append([math.copysign(magP*math.cos(headTheta), headV[0]), math.copysign(magP*math.sin(headTheta), headV[1]), headV[0], headV[1], 1, 1])
    ...
    while(usb.isconnected()):
        ...
        maxButt = 0
        maxHead = 0
        for blob in img.find_blobs(thresholds, pixels_threshold=6, area_threshold=10):
            if blob.code() == buttColor and (blob.area())  > maxButt: #butt
                maxButt = blob.area()
                butt = blob
                #img.draw_keypoints([(blob.cx(), blob.cy(), int(math.degrees(blob.rotation())))], size=20)
        ...
        if maxButt*maxHead > 0:
            ...
            if headMagO*1.1 > buttMagO:
                dot = sum([a*b for a,b in zip(headVO, bodyV)])
                bodyMag = math.sqrt(sum([math.pow(v, 2) for v in bodyV]))
                magP = dot/bodyMag
                out = [math.copysign(magP*math.cos(headTheta), headV[0]), math.copysign(magP*math.sin(headTheta), headV[1]), headV[0], headV[1], bodyV[0], bodyV[1]]
                out = array.array('d', [a/b for a,b in zip(out, maxAmp)])
                #out[0] /= maxAmp[0]
                #out[1] /= maxAmp[1]
