...
while(True):
    ...
    if maxButt*maxHead > 0:
        ...
        mag1 = math.sqrt(sum([math.pow(v, 2) for v in v1])) - ring.r()
        theta = math.atan(v1[1]/v1[0])
        v1 = [math.copysign(mag1*math.cos(theta), v1[0]), math.copysign(mag1*math.sin(theta), v1[1])]
        img.draw_line(head.cx(), head.cy(), head.cx() - round(v1[0]), head.cy() - round(v1[1]), color = (50, 50, 0), thickness = 3)
        
        v2 = [head.cxf() - butt.cxf(), head.cyf() - butt.cyf()]
        mag2 = math.sqrt(sum([math.pow(v, 2) for v in v2]))
        ...
        if magP > 0:
            ...
            img.draw_line(butt.cx(), butt.cy(), butt.cx() + round(v2[0]), butt.cy() + round(v2[1]), color = (0, 255, 50), thickness = 3)            
            # print(out)
        print(theta)
        print(dot)
        print(magP)
